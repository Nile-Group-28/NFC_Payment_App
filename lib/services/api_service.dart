...
class Session {
  ...
  static void clear() { _token = null; _user = null; }
...
class AuthApi {
  static Future<Map<String, dynamic>> register({
    ...
    final data = await _post('/auth/register', {
      'name': name, 'pin': pin, 'deviceId': deviceId,
  ...
  static Future<Map<String, dynamic>> setPassword({
    ...
    required String currentPin,
  }) => _post('/auth/set-password', {'password': password, 'currentPin': currentPin});
...
// ── KYC API ─────────────────────────────────────────────────────────────────────
class KycApi {
  static Future<Map<String, dynamic>> submitTier1({String? bvn, String? nin, String? dateOfBirth}) =>
      _post('/kyc/tier1', {
        if (bvn != null)         'bvn':         bvn,
        if (nin != null)         'nin':         nin,
        if (dateOfBirth != null) 'dateOfBirth': dateOfBirth,
  ...
  static Future<Map<String, dynamic>> submitTier2({
    required String bvn, required String nin, required String documentType,
  }) => _post('/kyc/tier2', {'bvn': bvn, 'nin': nin, 'documentType': documentType});

  static Future<Map<String, dynamic>> submitTier3({required String documentType}) =>
...
class WalletApi {
  ...
  static Future<Map<String, dynamic>> getTransactions({
    ...
    final q = [if (type != null) 'type=$type', 'limit=$limit', 'offset=$offset'].join('&');
  ...
  static Future<Map<String, dynamic>> nfcSettle({
    ...
    'senderId': senderId, 'amount': amount,
  ...
  }) => _post('/wallet/withdraw', {'amount': amount, 'bankAccountId': bankAccountId});
...
// ── Offline API ───────────────────────────────────────────────────────────────────
class OfflineApi {
  static Future<Map<String, dynamic>> enableOfflineCapability({
    required bool enable,
    required String deviceId,
    String? publicKey,
    double? dailyLimit,
  }) => _post('/wallet/offline-capability', {
    'enable':   enable,
    'deviceId': deviceId,
  ...
  static Future<Map<String, dynamic>> syncOfflineTransactions({
    required List<Map<String, dynamic>> transactions,
  }) => _post('/wallet/sync-offline-transactions', {'transactions': transactions});

  static Future<Map<String, dynamic>> getOfflineStatus({String? deviceId}) =>
      _get('/wallet/offline-status${deviceId != null ? '?deviceId=$deviceId' : ''}');
}
